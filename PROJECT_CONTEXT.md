...
**Last verified:** 2026-07-13, against commit `18d25dd` on branch `main`, updated for Phase 3 Sprint 3.1 (secure AI backend foundation) and Sprint 3.2 (real camera capture + mobile-to-backend integration).
...
- **`AddEditBirdScreen.tsx` still has its own separate, untouched mock photo-capture** (Bird & Flock management module, §5.2) — a `photoCaptured` boolean that stores the literal string `'captured'` as `photoUri` when editing an *existing* bird's photo. This is a different screen from onboarding's `AddFirstBirdScreen` and was explicitly out of scope for Sprint 3.2 (which targeted the onboarding flow only) — a reasonable follow-up would be applying the same real-capture pattern (`imagePickerService`) here too.
...
- **`AddEditBirdScreen.tsx`'s photo capture is still the old mock** (see §6) — a known, deliberately out-of-scope gap from Sprint 3.2.
...
- **Development stage:** Phase 2 (7 feature sprints, local-only client) is complete. Phase 3 is in progress: Sprint 3.1 (Secure AI Backend Foundation) and Sprint 3.2 (Real Camera Capture + Mobile-to-Backend Integration) are both complete — see §14 for full detail.
...
- **What is currently being worked on:** Nothing in-flight — Sprint 3.2 is complete, quality-gated, and committed. The repository is in a clean, stable state.
