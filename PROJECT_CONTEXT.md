...
| `tasks` | `id, birdId, flockId, type, title, description, dueDate, repeatType, completed, completedAt, notificationEnabled, notificationId, createdAt, updatedAt` | `birdId` → `birds(id)` `ON DELETE CASCADE`; `flockId` `ON DELETE SET NULL`. Doubles as the app's generic reminder engine — see §5.9, "Care Reminders" |
...
### 5.9 Shared UI component library
...
- **What is currently being worked on:** Nothing in-flight — the Vet Document Attachment sprint is complete, quality-gated, and committed as `0db0302` on `main`. The repository is in a clean, stable state.
