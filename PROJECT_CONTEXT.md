...
| `health_records` | `id, birdId, flockId, type, title, notes, medicine, dosage, startDate, endDate, time, veterinarian, cost, reminderDate, status, notificationId, createdAt, updatedAt` | `birdId` `ON DELETE CASCADE`, now **nullable** (v9) so a record can be logged against a flock instead of a specific bird. `flockId` (v9, new) `ON DELETE SET NULL`. `time` (v9, new) nullable `HH:MM`. A record must have at least one of `birdId`/`flockId` set — enforced in `AddEditHealthRecordScreen`, not a DB constraint |
...
### 5.8 Camera / Add tab wiring (`CameraSheetScreen`)
- Files: `CameraSheetScreen.tsx` (updated), `navigation/flockTypes.ts` (`AddEditHealthRecord` route param widened), `AddEditHealthRecordScreen.tsx` (reads the new param).
- All 7 capture-target buttons named in constitution §5 now do something except one. Each wired button cross-tab-navigates (`navigation.dispatch(CommonActions.navigate(...))`, the same pattern established by the earlier Egg/Hatch tray wiring and by `PulseHomeScreen`'s bird-profile jump) straight into an existing, already-built create screen — no new screens, tables, or persistence were added:
  ...
  - **Medicine** → `AddEditHealthRecord` with a new optional `presetType: 'treatment'` route param, so the form opens with "Medication" pre-selected in the type picker (still fully editable — the user confirms, nothing is silently forced)
  ...
  - **Vet document** → still unwired. No document/attachment/file-storage table exists anywhere in the schema (§4); adding one is a genuine new-feature scope, not a navigation fix, so it was deliberately left as-is rather than faked.
- `presetType` is the first route param ever added to `AddEditHealthRecord`; it only ever sets the initial `form.type` in create mode (`!isEditing`) and is otherwise inert — editing an existing record ignores it entirely.
- **Validation performed this sprint:** `tsc --noEmit` (clean), the unused-import/variable sweep (only the same 5 pre-existing, unrelated findings listed in §10), `expo-doctor` (18/18), `expo export --platform android` (succeeded), and `npm audit` (identical 12 moderate findings to the documented baseline — zero new, since no dependency was added or changed this sprint). No security-relevant surface was touched: no new permission, no new network call, no new dependency, no backend change — pure client-side navigation wiring plus one new optional, non-secret route param.
...
- **Camera / Add tab (`CameraSheetScreen`):** 6 of the 7 capture-target buttons (Bird, Egg, Medicine, Feed, Health concern, Hatch tray) now navigate into existing create screens (see §5.8) — only **Vet document** remains an inert placeholder, since no document/attachment storage exists anywhere in the schema. Note this screen still has no camera UI of its own — none of these buttons actually take a photo; they just jump to the relevant form, where photo capture (birds only, via `imagePickerService`) already exists inside that form itself. `expo-image-picker` exists in the project (added Sprint 3.2 for onboarding) but `CameraSheetScreen` itself still doesn't invoke it directly.
...
- **Camera/Add tab's "Vet document" button remains unwired** (see §5.8/§6) — the other 6 buttons were wired in the Camera/Add Tab Wiring sprint. This one specifically needs a document/attachment storage design (new table, likely `expo-file-system` + a new repository) before it can go anywhere, which is new-feature scope, not a navigation fix.
...
- **What is currently being worked on:** Nothing in-flight — the Camera/Add Tab Wiring sprint is complete, quality-gated, and committed as `b1e46eb` on `main`. The repository is in a clean, stable state.
