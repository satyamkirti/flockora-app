...
### 2026-07-13 — Phase 3 Sprint 3.2: Real Camera Capture + Mobile-to-Backend Bird Analysis Integration

The mobile client is now wired to the Sprint 3.1 backend for the first time — real camera/gallery capture replaces the onboarding mock, and the captured photo is uploaded to the existing `POST /api/v1/analyze-bird` endpoint. `MockAIProvider` is still the only provider on the backend (unchanged from Sprint 3.1); this sprint adds no AI provider, no AI provider key, and no billing risk. Full detail in `PROJECT_CONTEXT.md` §14.7.

Controls implemented and verified this sprint:
- **No direct AI-provider calls from the client** (Rules 3, 4): the client's entire codebase contains exactly one `fetch()` call (`src/services/birdAnalysisService.ts`), and it targets only `getBackendBaseUrl()` + `/api/v1/analyze-bird` — Flockora's own backend, never a third-party AI provider domain. Verified by grepping the full client source for `fetch(`/`XMLHttpRequest` (one match, as above) and for AI-provider domain strings / API-key patterns (none found).
- **`EXPO_PUBLIC_BACKEND_URL` — first use of `EXPO_PUBLIC_*` in this codebase, and it is compliant with Rule 2 by construction**: it holds only an optional backend base URL override, never a secret. It is documented as such inline in `src/config/backendConfig.ts`, and the backend itself performs all of its own request validation regardless of which URL reached it — the client-side URL is not a trust boundary.
- **No secrets added** (Rules 1, 5, 9): grepped the full client diff for key/token/secret/password patterns and common AI-provider API key prefixes (`AIza`, `sk-`, etc.) — none found. No `.env` file was added to the client (it never needed one before this sprint and still doesn't).
- **No sensitive logging introduced** (Rule 8): grepped the full client `src/` tree for `console.*` — zero matches, unchanged from baseline. Backend error response bodies are read only to extract a non-sensitive `code`/`message` pair already designed by the backend to be client-safe (see Sprint 3.1's centralized error handler); the raw response is never logged.
- **New Android/iOS permissions justified per Rule 12**: added camera and photo-library permissions via the `expo-image-picker` Expo config plugin in `app.json`, with custom, specific usage-description strings (not generic defaults) explaining exactly why Flockora needs each one. `microphonePermission` was explicitly set to `false` in the same plugin config to prevent the library's default behavior of also requesting `RECORD_AUDIO` on Android — Flockora only captures still photos, never audio/video, so that permission was deliberately suppressed rather than left in by default.
- **No blind dependency upgrades** (Rule 13): the two new dependencies (`expo-image-picker`, `expo-constants`) were added via `npx expo install`, which resolves the exact version compatible with the already-installed Expo SDK 54, rather than a manually guessed version. `npm audit` before and after shows the identical 12 moderate vulnerabilities already documented in the Audit Baseline — zero new vulnerabilities were introduced, and no existing dependency was upgraded.
- **Client-side request timeout**: `AbortController`-based 20-second timeout in `birdAnalysisService.ts`, complementing the backend's own `REQUEST_TIMEOUT_MS` guard from Sprint 3.1 — the client will not hang indefinitely even if the backend itself becomes unresponsive.

**Known related gap, deliberately not fixed this sprint**: `flockora-app/src/screens/AddEditBirdScreen.tsx` (a separate, already-shipped Bird Management screen, unrelated to onboarding) still has its own independent mock photo-capture placeholder (`photoCaptured` boolean, stores the literal string `'captured'` as `photoUri`). This sprint's instructions scoped the work to the onboarding flow specifically; touching a different, already-completed feature module was out of scope. Tracked in `PROJECT_CONTEXT.md` §6 as a known follow-up, not a security issue (no network/secret involved, purely a UI mock).

**Not yet implemented, explicitly out of scope for this sprint**: any real AI provider, per-user/global usage controls, authentication, and backend deployment — all unchanged from Sprint 3.1's "remaining work" list. This sprint does not, by itself, trigger a mandatory Rule 15 re-audit: it connects the client to the *already-audited, still-mocked* backend, and does not introduce real AI integration, backend integration (that happened in 3.1), authentication, cloud sync, or payments — the five specific Rule 15 triggers. The next sprint that adds any of those five will require one.

### 2026-07-14 — Notification & Reminder System — Production Hardening

A full audit-and-harden pass over the existing local-notification system (`expo-notifications`, already covered by the Audit Baseline below — this sprint added no new dependency and no new platform permission). Full technical detail in `PROJECT_CONTEXT.md` §5.17.

- **No new attack surface** (Rules 1–9, 12): no backend call, no analytics, no cloud sync, no authentication, no new Android/iOS permission (deep-linking, categories, and the preview UI are all built from `expo-notifications`, already justified and granted, plus existing React Navigation primitives), no secret of any kind. Verified by grepping every changed/new file for `console.*`, `EXPO_PUBLIC_*`/`process.env`, `fetch`/`axios`/`XMLHttpRequest`, `http(s)://`, and secret/token/password patterns — zero matches.
- **No new persistent storage**: notification identifiers are non-sensitive internal strings already stored in existing SQLite columns (`notificationId`, `candlingNotificationId`, etc.) — no new column, no new table, no `AsyncStorage`/`SecureStore` usage introduced.
- **Bird/task/health/feed/clutch deletion now correctly cancels every associated OS notification first** (previously, deleting a feed item or a bird could leave a scheduled notification pointing at a now-deleted record — an orphaned-notification bug, not a security vulnerability, but closed as part of this sprint's correctness audit; see `PROJECT_CONTEXT.md` §5.17 for the full list).
- **Deep-linking dispatches only to existing, already-reachable in-app routes** (`TaskDetail`, `HealthRecordDetail`, `FeedInventory`, `ClutchDetail`) using ids read from a notification's own `data` payload — the same ids the app itself wrote when scheduling that notification. No externally-supplied or remote data is ever used to construct a navigation target.
- **No blind dependency upgrade** (Rule 13): no dependency was added, removed, or upgraded this sprint. `npm audit` before/after is identical (12 moderate, unchanged baseline).

### 2026-07-14 — Backup/Restore & SQLite Integrity — Production Hardening

A data-integrity and crash-recovery hardening pass over the backup/export/import/restore system (`expo-sqlite`, already covered by the Audit Baseline below — this sprint added no new dependency and no new platform permission). Full technical detail in `PROJECT_CONTEXT.md` §5.18. This is a correctness/data-safety sprint, not a new-attack-surface sprint — recorded here because it touches the same restore path Rule 10's unencrypted-health-data risk already applies to, and because this document requires every architectural change to be logged.

- **No new attack surface** (Rules 1–9, 12): no backend call, no analytics, no cloud sync, no authentication, no new Android/iOS permission, no secret of any kind. The only new import across the diff is `Platform` from `react-native` (used for a `Platform.OS === 'web'` capability check, not a permission or network API). Verified by grepping every changed file for `console.*`, `EXPO_PUBLIC_*`/`process.env`, `fetch`/`axios`/`XMLHttpRequest`, `http(s)://`, and secret/token/password patterns — zero matches.
- **No new persistent storage or data collection**: the changes are entirely to how the existing on-device backup/restore path validates its own file and manages its own SQLite transaction — no new column, no new table, no new file location, nothing sent off-device. The unencrypted-at-rest posture of `health_records` (Rule 10) is unchanged; restored data lands in the exact same local SQLite file as before.
- **Restore transaction isolation hardened**: `backupRepository.restoreAllTables` (the one operation that wipes and repopulates every table) now runs on `db.withExclusiveTransactionAsync` instead of the non-exclusive `db.withTransactionAsync` — closing a real concurrency gap where a competing read or write elsewhere in the app could interleave with an in-progress restore. This is a data-integrity fix, not a confidentiality/authorization fix; it does not change what data exists or who can reach it, only guarantees the restore that does happen is atomic.
- **Malformed/corrupted backup files are now rejected before any write**: new `hasReferentialIntegrity()` (`utils/backupValidation.ts`) checks every foreign-key relationship in the backup against its own id sets, and `isValidBackupData()`'s row check was tightened to require a well-formed object with a numeric `id`. Both run against untrusted, user-supplied file content (a backup picked via `expo-document-picker` from anywhere on the device) purely in memory, with no `eval`/dynamic code execution and no SQL built from unvalidated field names beyond the pre-existing, already-reviewed column-driven `INSERT` pattern (columns come from the backup's own JSON keys, values are always bound as parameters, never string-interpolated).
- **No blind dependency upgrade** (Rule 13): no dependency was added, removed, or upgraded this sprint. `npm audit` before/after is identical (12 moderate, unchanged baseline).

### 2026-07-15 — UI Correctness — Double-Submission Prevention on Save/Confirm Buttons — Production Hardening

A client-side UI-correctness fix (`PROJECT_CONTEXT.md` §5.20) closing a duplicate-record risk on 13 Save/Confirm form screens — no backend, no network, no new dependency, no new permission. Recorded here per this document's own standing rule (§14) that every architectural change gets logged, even a UI-only one with no confidentiality/authorization surface change.

- **No new attack surface** (Rules 1–9, 12): every change is internal to one existing shared component (`PrimaryButton`) and its 13 existing call sites, all already covered by the Audit Baseline. The only behavioral change is that `Pressable`'s own `disabled` prop is now actually wired to the screen's existing `saving` state, instead of only a `style` prop dimming it. Verified by grepping the full diff for `console.*`, `EXPO_PUBLIC_*`/`process.env`, `fetch`/`axios`/`XMLHttpRequest`, `http(s)://`, and secret/token/password patterns — zero new matches (the pre-existing, already-documented `EXPO_PUBLIC_BACKEND_URL`/backend-fetch references are unrelated to this diff and unchanged).
- **Data-integrity fix, not a confidentiality/authorization fix**: prevents a fast double-tap on a Save/Confirm button from re-firing its handler while the first call is still in flight, which could previously create a duplicate database row (bird, task, health record, egg log, feed item, breeding pair, clutch, or candling/hatch record) with no error shown. Does not change what data exists once correctly saved, who can reach it, or any permission/auth boundary (none exist in this app yet — §Baseline).
- **No blind dependency upgrade** (Rule 13): no dependency was added, removed, or upgraded this sprint. `npm audit` before/after is identical (12 moderate, unchanged baseline).

### 2026-07-14 — Performance & Scalability — Production Hardening

A rendering/query/startup/memory performance audit (`PROJECT_CONTEXT.md` §5.19) — no backend, no network, no new dependency, no new permission. Recorded here per this document's own standing rule (§14) that every architectural change gets logged, even a performance-only one with no security-relevant surface change.

- **No new attack surface** (Rules 1–9, 12): every change is internal to existing screens/hooks already covered by the Audit Baseline — list virtualization (`FlatList` in place of `ScrollView`+`.map()`), `useCallback`-stabilized render functions, and removing one redundant `useEffect` from 4 data-fetching hooks. No new import beyond React/React Native primitives already used throughout the codebase (`useCallback`, `FlatList`). Verified by grepping the full diff for `console.*`, `EXPO_PUBLIC_*`/`process.env`, `fetch`/`axios`/`XMLHttpRequest`, `http(s)://`, and secret/token/password patterns — zero matches.
- **No behavior change to what data is shown or to whom**: this is a rendering-efficiency and duplicate-query-elimination pass over data the app already fetched and displayed — the fix removing a duplicate database read (§5.19, item 3) does not change *what* is fetched, only that it's fetched once instead of twice per trigger; verified against `@react-navigation/core`'s own source before making the change, not assumed.
- **No blind dependency upgrade** (Rule 13): no dependency was added, removed, or upgraded this sprint. `npm audit` before/after is identical (12 moderate, unchanged baseline).
